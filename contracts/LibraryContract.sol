...
contract LibraryContract {
    ...
    constructor() public {
        contractName = "Camerino Decentralized Library";
    }
    ...
    function purchaseBook(uint256 _bookId) public payable {
        ...
        require(
            _book.bookId > 0 && _book.bookId <= booksCount,
            "wrong Book id"
        );

        //require that it has enough Ether
        require(
            msg.value >= _book.bookPrice,
            "not enough token (whatever we call this token later, maybe CamiToken)"
        );
